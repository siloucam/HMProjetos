...
public class ExtendUserResource {
    ...
    @Timed
    public ResponseEntity<List<ExtendUser>> getAllExtendUsers(ExtendUserCriteria criteria) {
        log.debug("REST request to get ExtendUsers by criteria: {}", criteria);
        List<ExtendUser> entityList = extendUserQueryService.findByCriteria(criteria);
